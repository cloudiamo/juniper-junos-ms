...
if [[ -L /opt/fmc_repository/CommandDefinition/juniper-junos-ms ]]; then
	SYMLINK_TARGET=$(readlink -f /opt/fmc_repository/CommandDefinition/juniper-junos-ms || true)
	if [[ -n "$SYMLINK_TARGET" && -e "$SYMLINK_TARGET/.git" ]]; then
		log_info "🌹 Skipping upgrade for fellow developer."
		exit 0
	fi
	log_info "🦥 Removing symlink."
	rm -f /opt/fmc_repository/CommandDefinition/juniper-junos-ms
fi

# for backward compatibility with old backend, move existing repository to CommandDefinition
if [[ -e /opt/fmc_repository/juniper-junos-ms/.git ]]; then
	...
	if [[ -e /opt/fmc_repository/CommandDefinition/juniper-junos-ms ]]; then
		log_info "🍄 Removing existing destination directory before move."
		rm -rf /opt/fmc_repository/CommandDefinition/juniper-junos-ms
	fi
	mv /opt/fmc_repository/juniper-junos-ms /opt/fmc_repository/CommandDefinition/juniper-junos-ms
elif [[ -d /opt/fmc_repository/juniper-junos-ms ]]; then
	log_info "🐞 Not a git repository. Removing the directory for backend compatibility."
	rm -rf  /opt/fmc_repository/juniper-junos-ms
fi

if [[ -e /opt/fmc_repository/CommandDefinition/juniper-junos-ms/.git ]]; then
	log_info "🦔 Skipping upgrade for fellow developer."
	exit 0
fi
